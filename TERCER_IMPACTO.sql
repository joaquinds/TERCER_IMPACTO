...
CREATE PROCEDURE TERCER_IMPACTO.COMPRAR_BONOS_FAMILIA
(@ID_AFIL NUMERIC(18,0), @NUM_FAM NUMERIC(18,0),@CANT INT,@FECHA DATETIME)
AS
BEGIN
 DECLARE @I INT;
 SET @I='0';
 ...
 WHILE @I<@CANT
  ...
  --
   INSERT INTO TERCER_IMPACTO.BONO
   (ID_PLAN_MEDICO,ID_AFILIADO,NUM_FAMILIA, FECHA,COMPRA_BONO_ID)
   VALUES ((SELECT ID_PLAN_MEDICO FROM
   AFILIADO WHERE ID_AFILIADO=@ID_AFIL),
   @ID_AFIL,@NUM_FAM,@FECHA,@ID_COMPRA)
   SET @I=@I+1;
  END
     --
...
CREATE FUNCTION TERCER_IMPACTO.TIENE_FAMILIA
...
BEGIN
 ...
 IF (EXISTS(SELECT 1 FROM TERCER_IMPACTO.AFILIADO WHERE NUM_FAMILIA IS NOT NULL AND ID_AFILIADO = @AFILIADO))
...
(@TURNO NUMERIC(18,0), @FECHA DATETIME,@AFILIADO NUMERIC(18,0))
AS
BEGIN
	DECLARE @ID_C NUMERIC(18,0)
	SET @ID_C=(SELECT TOP 1 ID_CONSULTA FROM TERCER_IMPACTO.CONSULTA
	WHERE ID_TURNO=@TURNO AND FECHA = @FECHA)
	
	IF (SELECT NUM_FAMILIA FROM TERCER_IMPACTO.AFILIADO
	WHERE ID_AFILIADO=@AFILIADO) IS NOT NULL
		BEGIN
			DECLARE @NUM_FLIA NUMERIC(18,0);
			SELECT @NUM_FLIA=NUM_FAMILIA FROM 
			TERCER_IMPACTO.AFILIADO WHERE ID_AFILIADO=@AFILIADO
			UPDATE TOP (1) TERCER_IMPACTO.BONO SET ID_CONSULTA = @ID_C
			WHERE ID_CONSULTA IS NULL AND NUM_FAMILIA=@NUM_FLIA
		END
	ELSE
		BEGIN
			UPDATE TOP (1) TERCER_IMPACTO.BONO SET ID_CONSULTA = @ID_C
			WHERE ID_CONSULTA IS NULL AND ID_AFILIADO=@AFILIADO
		END
END
GO
