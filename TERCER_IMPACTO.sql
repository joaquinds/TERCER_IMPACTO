...
CREATE PROCEDURE TERCER_IMPACTO.CANCELAR_TURNO_PROF (@FECHA DATETIME, @EXPLICACION VARCHAR(255))
AS
BEGIN
	...
	MONTH(FECHA)=YEAR(@FECHA) AND
	DAY(FECHA)= YEAR(@FECHA)
...
CREATE PROCEDURE TERCER_IMPACTO.CANCELAR_TURNO_PROF_PERIODO (@FECHA DATETIME, @EXPLICACION VARCHAR(255), @ID_MED NUMERIC(18,0) )
AS
BEGIN
	...
	YEAR(FECHA)=YEAR(@FECHA) AND
	MONTH(FECHA)=YEAR(@FECHA) AND
	DAY(FECHA)= YEAR(@FECHA) AND ID_MEDICO = @ID_MED
