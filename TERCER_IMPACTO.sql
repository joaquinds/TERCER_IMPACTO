...
AS
	RETURN(
		SELECT TOP 5 P.MATRICULA CODIGO,AFI.NOMBRE NOMBRE,AFI.APELLIDO APELLIDO,E.DESCRIPCION ESPECIALIDAD, count(*)/2 HORAS_TOTAL_TRABAJADAS
			   FROM TERCER_IMPACTO.TURNO T,
			...
			AND YEAR(T.FECHA)=@ANIO
			AND MONTH(T.FECHA) BETWEEN @MES_INICIAL AND @MES_FINAL
			GROUP BY AFI.NOMBRE,AFI.APELLIDO,P.MATRICULA ,P.MATRICULA,E.DESCRIPCION
			ORDER BY COUNT(*) ASC
			)
GO
