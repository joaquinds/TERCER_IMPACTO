...
(@PASS VARCHAR(255),@HASH VARCHAR(255)) 
RETURNS BIT
AS
BEGIN
	DECLARE @RESULT BIT;
	IF (@HASH = HASHBYTES('SHA2_256',@PASS))
		SET @RESULT=1;
	ELSE
		SET @RESULT=0;
...
set @HASH=(SELECT PASS 
FROM TERCER_IMPACTO.USUARIO WHERE USERNAME='admin');

select TERCER_IMPACTO.comparar_HASH('w32e',@HASH)
SELECT top 1 ID_USUARIO, PASS, INTENTOS_FALLIDOS FROM TERCER_IMPACTO.USUARIO WHERE USERNAME ='admin'
